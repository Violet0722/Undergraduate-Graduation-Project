...
import torch
from pytracking.save_image import save_bbox
...
class Tracker:
    ...
    def _track_sequence(self, tracker, seq, init_info, debug):
        ...
        init_default = {'target_bbox': [1,1,1,1],
        ...
        for frame_num, frame_path in enumerate(seq.frames[1:], start=1):
            ...
            if debug == 0:
                if frame_num == start_frame:
                    init_info['init_bbox'] = seq.ground_truth_rect[frame_num]
                    tracker.initialize(image, init_info)
                    _store_outputs(pred_out, {'target_bbox': [1,1,1,1],
                                        'time': time.time() - start_time})
                elif frame_num > start_frame:
                    gt_bbox = seq.ground_truth_rect[frame_num].tolist()
                    output_dict = tracker.track(image, info)
                    state = output_dict['target_bbox']
                    overlap = IoU(gt_bbox, state)
                    # print("--------------------",overlap)
                    if overlap <= 0:
                        # continue tracking
                        state = [2, 2, 2, 2]
                    ...
                    _store_outputs(pred_out, {'target_bbox': state,
                                         'time': time.time() - start_time
                                         })
                else:
            ...
            if self.visdom is not None:
                tracker.save_box(image, out['target_bbox'], seq.ground_truth_rect[frame_num].tolist(), frame_num, seq.name, self.name, out['flag'])
