...
def sample_fs(a: torch.Tensor, grid_sz: torch.Tensor = None, rescale = True):
    ...
    print("傅立叶里的操作----------------tot_pad", tot_pad)
    is_even = [s.item() % 2 == 0 for s in sz]
    print("傅立叶里的操作----------------is_even", is_even)
    # Compute paddings
    pad_top = int((tot_pad[0]+1)/2) if is_even[0] else int(tot_pad[0]/2)
    pad_bottom = int(tot_pad[0] - pad_top)
    pad_right = int((tot_pad[1]+1)/2)
    print("傅立叶里的操作----------------pad{}---{}---{}".format(pad_top, pad_bottom, pad_right))
    print("傅立叶里的操作----------------填充后", cifft2(F.pad(a, (0, 0, 0, pad_right, pad_top, pad_bottom)), signal_sizes=grid_sz.long().tolist()).shape)
    if rescale:
