...
from pytracking.save_image import save_bbox, save_heat_map
...
class ATOM(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Set search area
        # search_area为边界框的五倍大小
        self.target_scale = 1.0
        search_area = torch.prod(self.target_sz * self.params.search_area_scale).item()
        #print(search_area, self.params.max_image_sample_size)
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # Convert image把图像ｎｕｍｐｙ类型转为ｔｅｎｓｏｒ类型
        im = numpy_to_torch(image)
        self.im = im    # For debugging only
        # ------- LOCALIZATION ------- #
        # Get sample
        sample_pos = self.pos.round()
        print("刚开始的self_pos----------------", self.pos)
        print("刚开始的sample_pos----------------", sample_pos)
        sample_scales = self.target_scale * self.params.scale_factors
        print("atom.py--image------{}-sample_pos------{}--sample_scales------{}".format(im.shape,sample_pos, sample_scales))
        ...
        print("scores_raw---------",scores_raw.size())

        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw)
        print("s---------", s.size(), scale_ind, flag, translation_vec)
        # Update position and scale 更新位置和尺度
        if flag != 'not_found':
            if self.use_iou_net:
                ...
                if self.params.get('use_classifier', True):
                    ...
                    print("更改后的self_pos----------------", self.pos, translation_vec)
                    print("更改后的sample_pos----------------", sample_pos)
        ...
        if self.visdom is not None:
            ...
            self.save_heatmap(score_map, self.frame_num,  self.seq_name)

        elif self.params.debug >= 2:
            show_tensor(score_map, 5, title='Max score = {:.2f}'.format(max_score))
        # ------- UPDATE ------- #
        # Check flags and set learning rate if hard negative检查标志并设置学习率（硬样本）
        ...
        print("pos位置  %s   目标sz %s" %(self.pos, self.target_sz))
    ...
    def localize_target(self, scores_raw):
        ...
        # 在傅立叶域中进行插值的加权和（如果有多个特征）
        weight = self.fparams.attribute('translation_weight', 1.0)
        print("weight=============={},======"
              "=========",weight, scores_raw.size())
        scores_raw = weight * scores_raw
        print("傅立叶变化以后的socre_raw================", fourier.cfft2(scores_raw).size())
        ...
        print("傅立叶变化以后的scores_fs================", scores_fs.size())
        scores = fourier.sample_fs(scores_fs, self.output_sz)

        print("------------看看你到底是个神马东西------------------------------------",scores.size())
    ...
    def localize_advanced(self, scores):
        ...
        # 是否未找到带有硬样本检测和目标的高级定位。
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        sz = scores.shape[-2:]
        print("sz--------------------", sz)
        print("output_window---------", self.output_window)
        ...
        print("1代码里的----------{}----------{}".format(max_score1, max_disp1))
        _, scale_ind = torch.max(max_score1, dim=0)
        max_score1 = max_score1[scale_ind]
        max_disp1 = max_disp1[scale_ind,...].float().cpu().view(-1)
        target_disp1 = max_disp1 - self.output_sz // 2
        print("2代码里的----------{}----------{}".format(max_score1, max_disp1))
        print("3代码里的----------{}----------{}".format(target_disp1, self.output_sz))
        print("代码里的----------{}----------{}".format(self.img_support_sz, self.output_sz))
        translation_vec1 = target_disp1 * (self.img_support_sz / self.output_sz) * self.target_scale
        print("4代码里的----------{}----------{}".format(translation_vec1, self.img_support_sz))
        if max_score1.item() < self.params.target_not_found_threshold:
            return translation_vec1, scale_ind, scores, 'not_found'

        """------------------------------------------------------------------"""
        # max_score, max_disp = dcf.top_k(scores, 5)
        # print("1我的----------{}----------{}".format(max_score, max_disp))
        # for i in enumerate(max_score):
        #     max_disp[i]
        # max_disp = max_disp[:].float().cpu().view(-1)
        # target_disp = max_disp - self.output_sz // 2
        # print("2我的----------{}----------{}".format(max_score, max_disp))
        # print("3我的----------{}----------{}".format(target_disp, self.output_sz))
        # translation_vec = target_disp *(self.img_support_sz / self.output_sz) * self.target_scale
        # print("4我的----------{}----------{}".format(translation_vec, self.img_support_sz))
        # if max_score.item() < self.params.target_not_found_threshold:
        #     return translation_vec1, scale_ind, scores, 'not_found'
        """------------------------------------------------------------------"""

        if self.output_window is not None and self.params.get('perform_hn_without_windowing', False):
            print("这个在这个是什么用啊，你进不进来？")
            scores = scores_orig

        # Mask out target neighborhood 屏蔽目标邻域，将目标其他位置为0
        ...
        print("5--------------------------{}".format(target_neigh_sz))
        print("6---------------------------{}----{}-----{}---{}".format(tneigh_top,tneigh_bottom, tneigh_left, tneigh_right))
        print("7--------------------------{}".format(scores_masked.shape))
        ...
        print("5代码里的----------{}----------{}".format(max_score2, max_disp2))
        print("5代码里的----------{}----------{}".format(target_disp2, translation_vec2))
        print("self____pos".format(self.pos))
        print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
        ...
        if max_score2 > self.params.distractor_threshold * max_score1:
            ...
            print("1--------------------第二个最大值的东西",disp_norm1)
            print("2--------------------第二个最大值的东西", disp_norm2)
            print("3--------------------第二个最大值的东西", disp_threshold)
    ...
    def extract_processed_sample(self, im: torch.Tensor, pos: torch.Tensor, scales, sz: torch.Tensor) -> (TensorList, TensorList):
        ...
        #print("xxxxxxx{}xx{}xxx{}".format(x.size(), self.project_sample(x).size(), self.preprocess_sample(self.project_sample(x)).size()))
    ...
    def refine_target_box(self, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        print("pos-----------------{}-----------------{}".format(self.pos, sample_pos))
        print("init_box----------", init_box)
        print("scale_inde--------", scale_ind)
        # Extract features from the relevant scale
        iou_features = self.get_iou_features()
        print("iou_featrures-----",  iou_features.size())
        iou_features = TensorList([x[scale_ind:scale_ind+1,...] for x in iou_features])
        print("iou_featrures-----", iou_features.size())
        init_boxes = init_box.view(1,4).clone()
        print("init_box----------", init_boxes)
        print("num_init_random_boxes----------", self.params.num_init_random_boxes)
        if self.params.num_init_random_boxes > 0:
            # Get random initial boxes
            "根据init_box得到num_init_random_boxes个随机的初始框"
        ...
        print("new_pos----------", self.pos_iounet)
        print("new_target_sz----------", new_target_sz)
    ...
    def visdom_draw_tracking(self, image, box, gt_box, segmentation=None):
        self.visdom.register((image, box, gt_box), 'Tracking', 1, 'Tracking')

    def save_box(self, image, box, gt_box, fram_num, seq_name, tracker_name):
        if hasattr(self, 'search_area_box'):
           save_bbox(image, box, gt_box, fram_num, seq_name, tracker_name, self.search_area_box)
        else:
            save_bbox(image, box, gt_box, fram_num, seq_name, tracker_name)

    def save_heatmap(self, score_map, fram_num, seq_name):
        save_heat_map(score_map, fram_num, seq_name)
