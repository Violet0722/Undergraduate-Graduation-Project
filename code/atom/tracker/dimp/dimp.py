...
class DiMP(BaseTracker):
    ...
    def initialize(self, image, info: dict) -> dict:
        ...
        # Extract and transform sample, 提取第一帧的特征信息（经过数据增强后的）
    ...
    def track(self, image, info: dict = None) -> dict:
        ...
        # sample_coords是288*target_scale(应该目标尺寸是五倍大小)的区域的坐标左上和右下角的坐标
        ...
        scores_raw, scores_raw2 = self.classify_target(test_x)

        # Localize the target
        translation_vec, scale_ind, s, flag = self.localize_target(scores_raw, sample_pos, sample_scales, dis_flag="hard_negative")

        if flag == 'uncertain':
            translation_vec2, scale_ind2, s2, flag = self.localize_target(scores_raw2, sample_pos, sample_scales, dis_flag="uncertain")

        #translation_vec2, scale_ind2, s2, flag2 = self.localize_target(scores_raw2, sample_pos, sample_scales)
        new_pos = sample_pos[scale_ind,:] + translation_vec



        # Update position and scale
        # 当flag为not_found时，位置和尺度不再变化
        if flag != 'not_found':
            if self.params.get('use_iou_net', True):
       ...
       # score_map2 = s2[scale_ind2, ...]
        ...
        if self.visdom is not None:
            ...
            # self.save_heatmap(score_map, self.frame_num, self.seq_name, tracker_name="dimp")
            # score_map2 = torch.squeeze(score_map2.cpu())
            # self.save_heatmap(score_map2, self.frame_num, self.seq_name, tracker_name="dimp_2")
    ...
    def classify_target(self, sample_x: TensorList):
        ...
        with torch.no_grad():
            scores2 = self.net.classifier.classify(self.target_filter_init, sample_x)
        return scores, scores2

    def localize_target(self, scores, sample_pos, sample_scales, dis_flag):
        ...
        if self.params.get('advanced_localization', False):
            return self.localize_advanced(scores, sample_pos, sample_scales, dis_flag)
    ...
    def localize_advanced(self, scores, sample_pos, sample_scales, dis_flag):
        ...
        if max_score1.item() < self.params.target_not_found_threshold:
            return translation_vec1, scale_ind, scores_hn, '' \
                                                           ''
        ...
        # t = max(round(max_disp1[0].item() - 4 / 2), 0)
        # b = min(round(max_disp1[0].item() + 4 / 2 + 1), sz[0])
        # l = max(round(max_disp1[1].item() - 4 / 2), 0)
        # r = min(round(max_disp1[1].item() + 4 / 2 + 1), sz[1])
        scores_masked_mean = torch.std(scores_masked[..., tneigh_top:tneigh_bottom, tneigh_left:tneigh_right])
        scores_masked[..., tneigh_top:tneigh_bottom, tneigh_left:tneigh_right] = 0
        print("----这个比较重要---[{},{}]--{}".format(tneigh_bottom - tneigh_top, tneigh_right - tneigh_left, scores_masked_mean))

        print("没改的gd%%%%%%%%%%%%%%%%%", self.gd)
        print("当前gd_update%%%%%%%%%%%%%%%%%", self.gd_update)
        flag = self.flag
        flag2 = False
        if dis_flag == "hard_negative":
            gd = 0.0
            if self.gd == 0.0:
                self.gd = scores_masked_mean
                self.gd_update = scores_masked_mean
                self.gd_dis = scores_masked_mean
            else:
                gd = self.gd - scores_masked_mean
                gd_abs = torch.abs(gd)
                gd_update = self.gd_update - scores_masked_mean
                print("%%%%%%%%%----%%%%%%%%", gd)
                self.gd_update = scores_masked_mean
                ...
                if gd_abs >= 0.03:
                   ...
                   flag = True
                    ...
                    pass
            if self.gd_update >= 0.19:
                self.gd = scores_masked_mean
            self.gd_update = scores_masked_mean
            print("现在的gd%%%%%%%%%%%%%%%%%", self.gd)
        elif dis_flag == "uncertain":
            dis_gd = torch.abs(self.gd_dis - scores_masked_mean)
            gd_update = self.gd_update - scores_masked_mean
            print(dis_gd)
            print(gd_update)
            if dis_gd > 0.05:
                ...
                flag = True
                if self.gd < 0.15 and gd_update >= 0.01:
                    flag2 = True
                else:
                    flag2 = False
            self.gd_dis = self.gd
        ...
        #print("%%%%%%%%%%%%%%%%%{}-------{}--------{}".format(dis_flag, self.gd_update, gd))
        # Find new maximum
        ...
        print("+++++++{}++{}++{}".format(max_score1, max_score2, prev_target_vec))
        ...
        print("------{}--{}".format(disp_norm1, disp_norm2))


        if disp_norm1 < disp_norm2:
            if flag:
                if flag2:
    ...
    def init_classifier(self, init_backbone_feat):
        ...
        # 初始化分类器以后不对这个分类器进行更新操作
        self.target_filter_init = self.target_filter
    ...
    def visdom_draw_tracking(self, image, box, gt_box, segmentation=None):
        if hasattr(self, 'search_area_box'):
            self.visdom.register((image, box, gt_box,  self.search_area_box), 'Tracking', 1, 'Tracking')
        else:
            self.visdom.register((image, box), 'Tracking', 1, 'Tracking')

    def save_box(self, image, box, gt_box, fram_num, seq_name, tracker_name, flag):
        if hasattr(self, 'search_area_box'):
           save_bbox(image, box, gt_box, fram_num, seq_name, tracker_name, flag, self.search_area_box)
        else:
            save_bbox(image, box, gt_box, fram_num, seq_name, tracker_name, flag)

    def save_heatmap(self, score_map, fram_num, seq_name, tracker_name):
        save_heat_map(score_map, fram_num, seq_name, tracker_name)